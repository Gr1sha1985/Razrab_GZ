...
Процедура ПриОпределенииНастроек(Настройки) Экспорт
	...
	Настройки.ОбщиеНастройкиВхода = ВариантыПриложений.ЭтоВерсияКОРП() И Не ОбщегоНазначения.РазделениеВключено()
...
Процедура ПриОпределенииНазначенияРолей(НазначениеРолей) Экспорт
	...
	//НазначениеРолей.ТолькоДляПользователейСистемы.Добавить(Метаданные.Роли.ОператорОтправкиОтчетностиЧерезПредставителяПереопределяемая.Имя);
	//НазначениеРолей.ТолькоДляПользователейСистемы.Добавить(Метаданные.Роли.ДобавлениеИзменениеОбщейБухгалтерскойНСИ.Имя);
	//НазначениеРолей.ТолькоДляПользователейСистемы.Добавить(Метаданные.Роли.ДобавлениеИзменениеНеразделяемыхДанныхОбменаСБанками.Имя);
...
// Если сейчас добавляется новый пользователь, а для всех ранее существующих пользователей начальная страница скрыта,
	// то и у нового пользователя по умолчанию ее тоже скроем.
	
	Если НЕ ПустаяСтрока(СтарыеСвойства.Имя) Тогда
		// Это не новый, а ранее существующий пользователь, его настройки не меняем.
		Возврат;
	КонецЕсли;
	...
	Если ОбщегоНазначенияБП.ЭтоИнтерфейсИнтеграцииСБанком() Тогда
		...
		СоставФорм.ЛеваяКолонка.Добавить("ОбщаяФорма.СписокЗадач");
		НачальнаяСтраница.УстановитьСоставФорм(СоставФорм);
		ХранилищеСистемныхНастроек.Сохранить("Общее/НастройкиНачальнойСтраницы", "", НачальнаяСтраница,, НовыеСвойства.Имя);
		
	ИначеЕсли ОбщегоНазначенияБП.ИспользуетсяСервисЭлектронныхТрудовыхКнижек() Тогда
	...
	Иначе
		...
		КоличествоСПустойНачальнойСтраницей = 0;
		КоличествоДругихПользователей = 0;
		...
		Для каждого КлючИЗначение Из НастройкиНачальнойСтраницыПользователей Цикл
			...
			Если ИмяПользователя = НовыеСвойства.Имя Тогда
				// Свои собственные настройки не учитываем.
				Продолжить;
			КонецЕсли;

			КоличествоДругихПользователей = КоличествоДругихПользователей + 1;
			ЕстьНачальнаяСтраница = Истина; // Если индивидуальных настроек у пользователя нет, то начальная страница показывает форму по умолчанию.
			...
			Если НЕ ЕстьНачальнаяСтраница Тогда
				КоличествоСПустойНачальнойСтраницей = КоличествоСПустойНачальнойСтраницей + 1;
			КонецЕсли;
		
		КонецЦикла;
		
		Если КоличествоДругихПользователей > 0 И КоличествоДругихПользователей = КоличествоСПустойНачальнойСтраницей Тогда
			ОбщегоНазначенияБП.ОтключитьНачальнуюСтраницуПользователя(НовыеСвойства.Имя);
		КонецЕсли;
...
//	НастройкиТакси.УстановитьСостав(ОбщегоНазначенияБП.НастройкиСоставаИнтерфейсаКлиентскогоПриложения());
