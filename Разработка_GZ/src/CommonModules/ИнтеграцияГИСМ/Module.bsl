...
Функция ПоддерживаемыеТипыДокументов() Экспорт
	...
	//ОписаниеТипаДокумента.Вставить("Описание", НСтр("ru = 'Уведомление об изменении cтатуса заявки на выпуск КиЗ'"));
...
Функция ПоследнийУказанныйВДокументахGLNКонтрагента(Контрагент) Экспорт
	...
	//ГИСМБП
	// Из запроса исключена обработка документа ЗаявкаНаВыпускКиЗ
	//КОНЕЦ ГИСМБП
	...
	|		И УведомлениеОПоступленииМаркированныхТоваровГИСМ.GLNКонтрагента <> """" ) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	Дата УБЫВ";
...
Функция ИспользоватьПодразделения() Экспорт
	...
	Использовать = Ложь;
	ИнтеграцияГИСМПереопределяемый.ИспользоватьПодразделения(Использовать);
