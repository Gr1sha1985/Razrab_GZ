...
Функция ПолучитьПараметрыИНастройкиМеханизмаОнлайнСервисовРО() Экспорт
	...
	ПараметрыИНастройки.Вставить("ЗапроситьПодтверждениеДоступаВИнтернет", ВерсияРелизаНаКоторомДоступВИнтернетЗапрещен <> ВерсияПрограммы());
	ПараметрыИНастройки.Вставить("НастройкиМеханизмаОнлайнСервисов", ПолучитьНастройкиМеханизмаОнлайнСервисовРО());
...
#КонецОбласти 
	...
	ОбновлениеТаблицыФорматовРосстата(Соединение);
...
Функция СобытиеНаступило(Знач ИмяСобытия) Экспорт
	
	Актуализировать = Ложь;
	Возврат СобытиеНаступилоРасширенная(ИмяСобытия, Актуализировать).СобытиеНаступило;
...
Процедура ОбновлениеТаблицыФорматовРосстата(Соединение)
	Попытка
		...
		ИмяВременногоКлючевогоФайла = ПолучитьРесурсССервера(Соединение, "stat_meta.xml", ОписаниеОшибки);
		Если ИмяВременногоКлючевогоФайла = Неопределено Тогда
			Возврат;
		КонецЕсли;
		СодержимоеКлючевогоФайла = ПрочитатьТекстовыйФайл(ИмяВременногоКлючевогоФайла, ОписаниеОшибки, Истина);
		...
		ЧтениеXML.УстановитьСтроку(СодержимоеКлючевогоФайла);
		ЧтениеXML.Прочитать();
		ИмяФайла = ""; ДатаПолучения = Дата("00010101");
		Если ЧтениеXML.Имя = "format_table_meta" Тогда 
			Пока Истина Цикл
				Если Не ЧтениеXML.ПрочитатьАтрибут() Тогда 
					Прервать;
				КонецЕсли;
				Если ЧтениеXML.Имя = "FName" Тогда
					ИмяФайла = ЧтениеXML.Значение;
				ИначеЕсли ЧтениеXML.Имя = "RefreshDate" Тогда 
					СтрД = СтрЗаменить(ЧтениеXML.Значение, "-", "");
					ДатаПолучения = Дата(Прав(СтрД,4) + Сред(СтрД,3,2) + Лев(СтрД,2));
				Иначе 
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		ЧтениеXML.Закрыть();
		Если Не ЗначениеЗаполнено(ИмяФайла) Или Не ЗначениеЗаполнено(ДатаПолучения) Тогда 
			Возврат;
		КонецЕсли;
		
		Выборка = РегистрыСведений.РесурсыМеханизмаОнлайнСервисовРО.Выбрать(Новый Структура("Тип", Перечисления.ТипыРесурсовМеханизмаОнлайнСервисовРО.ТаблицаВерсийФорматовРосстата));
		Если Выборка.Следующий() И Выборка.Имя = ИмяФайла Тогда 
			Возврат;
		КонецЕсли;
		
		ИмяВременногоФайлаСДанными = ПолучитьРесурсССервера(Соединение, "stat_" + ИмяФайла + ".zip", ОписаниеОшибки);
		Если ИмяВременногоФайлаСДанными = Неопределено Тогда
			Возврат;
		КонецЕсли;
		
		Архив = Новый ЧтениеZipФайла(ИмяВременногоФайлаСДанными);
		...
		Архив.Закрыть();
		СодержимоеФайлаСДанными = ПрочитатьТекстовыйФайл(ИмяФайлаИзАрхива, ОписаниеОшибки, Истина);
		Если СодержимоеФайлаСДанными = Неопределено Тогда
			Возврат;
		КонецЕсли;
		УдалитьФайл(ИмяФайлаИзАрхива);
		
		НЗ = РегистрыСведений.РесурсыМеханизмаОнлайнСервисовРО.СоздатьНаборЗаписей();
		НЗ.Отбор["Тип"].Значение = Перечисления.ТипыРесурсовМеханизмаОнлайнСервисовРО.ТаблицаВерсийФорматовРосстата;
		НЗ.Отбор["Тип"].Использование = Истина;
		НЗ.Прочитать();
		НЗ.Очистить();
		Запись = НЗ.Добавить();
		Запись.Тип = Перечисления.ТипыРесурсовМеханизмаОнлайнСервисовРО.ТаблицаВерсийФорматовРосстата;
		Запись.Данные = СодержимоеФайлаСДанными;
		Запись.Имя = ИмяФайла;
