...
Функция СформироватьСтруктуруОбязательныхРеквизитовПоПроцедуре_fcsnotificationOKD_(xdto_пакет, ИмяФайла) Экспорт 
	
	СтруктураДанныхДляИзвещения = Новый Структура;
	СтруктураДанныхДляИзвещения.Вставить("ИмяФайла",ИмяФайла);
	СтруктураДанныхДляИзвещения.Вставить("schemeVersion", xdto_пакет.schemeVersion);
	СтруктураДанныхДляИзвещения.Вставить("Дата",?(xdto_пакет.Свойства().Получить("docPublishDate")<> Неопределено, xdto_пакет.docPublishDate,ТекущаяДата()));
