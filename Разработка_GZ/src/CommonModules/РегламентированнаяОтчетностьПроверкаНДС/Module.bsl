...
// Помещает во временное хранилище представление ошибок, препятсвующих формированию электронного представления
// декларации по НДС. Предназначена для редакций декларации по НДС, соответствующих формам "ФормаОтчета2020Кв4",
// "ФормаОтчета2019Кв1" и "ФормаОтчета2017Кв1" отчета "РегламентированныйОтчетНДС".
// Параметры:
...
Процедура ПроверитьЗаполнение_Раздел1(КонтекстВыполнения)
	...
	Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4") Тогда
...
Процедура ПроверитьЗаполнение_Раздел2(КонтекстВыполнения)
	...
	Если ВРег(СокрЛП(КонтекстВыполнения.ИмяФормы)) = ВРег("ФормаОтчета2020Кв4")
		...
		ВозможныеКодыОпераций_Стр = "1011703, 1011705, 1011707, 1011709, 1011711, 1011712, 1011714, 1011715, 1011716, 1011717, 1011718";
	Иначе
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел8(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если ТребуетсяУказатьПродавца И НЕ УказаныСведенияОПродавце Тогда
		Если ВРег(КонтекстВыполнения.ИмяФормы) = ВРег("ФормаОтчета2020Кв4")
...
Процедура ПроверитьЗаполнениеСтрокиДанных_Раздел10(СтрокаДанных, НомерСтроки, КонтекстВыполнения)
	...
	Если СтрокаДанных.СвСчФОтПрод.Количество() = 0 Тогда
