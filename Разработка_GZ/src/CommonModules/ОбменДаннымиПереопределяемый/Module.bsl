...
// КОРП, ПРОФ
	...
	// КОРП
	Если Метаданные.ПланыОбмена.Найти("ОбменБухгалтерияПредприятияДокументооборот") <> Неопределено Тогда
		ПланыОбменаПодсистемы.Добавить(Метаданные.ПланыОбмена["ОбменБухгалтерияПредприятияДокументооборот"]);
	КонецЕсли;
	Если Метаданные.ПланыОбмена.Найти("ОбменБухгалтерияПредприятияДокументооборот20") <> Неопределено Тогда
	...
	КонецЕсли;
	Если Метаданные.ПланыОбмена.Найти("ОбменРозница1Бухгалтерия3") <> Неопределено Тогда
...
Процедура РегистрацияИзмененийНачальнойВыгрузкиДанных(Знач Получатель, СтандартнаяОбработка, Отбор) Экспорт
	...
	Если ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(Получатель) = "ОбменУправлениеТорговлейБухгалтерияПредприятияКОРП30" // КОРП
		ИЛИ ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(Получатель) = "ОбменУправлениеТорговлейБухгалтерияПредприятия30"  // ПРОФ
		ИЛИ ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(Получатель) = "СинхронизацияДанныхЧерезУниверсальныйФормат" Тогда // КОРП, ПРОФ
		ОбменДаннымиСобытияБП.РегистрацияИзмененияДляНачальнойВыгрузки(Получатель, СтандартнаяОбработка, Отбор);
	КонецЕсли;
...
Процедура ПриПолученииДоступныхВерсийФормата(ВерсииФормата) Экспорт
	...
	ВерсииФормата.Вставить("1.7",      МенеджерОбменаЧерезУниверсальныйФормат13);
	ВерсииФормата.Вставить("1.8",      МенеджерОбменаЧерезУниверсальныйФормат13);
