...
Функция ФормаВыполненияЗадачи(Знач ЗадачаСсылка) Экспорт
	...
	Если Реквизиты.БизнесПроцесс = Неопределено ИЛИ Реквизиты.БизнесПроцесс.Пустая() Тогда
		...
		ФормаДляПустогоБизнесПроцесса(СтруктураФормы);
...
// Переопределим открытие формы для случая пустого бизнес-процесса.
Процедура ФормаДляПустогоБизнесПроцесса(СтруктураФормы) 
	
	СтруктураФормы.Вставить("ИмяФормы", "ФормаЗадачиБП");
	СтруктураФормы.Вставить("ПараметрыФормы", Новый Структура());
