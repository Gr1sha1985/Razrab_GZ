...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ГЗ_ИзвещениеОПроведенииЗакупок.purchaseObject КАК purchaseObject,
		|	СУММА(ГЗ_ИзвещениеОПроведенииЗакупок.sum) КАК sum
		|ИЗ
		|	Документ.ГЗ_ИзвещениеОПроведенииЗакупок.purchaseObjects КАК ГЗ_ИзвещениеОПроведенииЗакупок
		|ГДЕ
		|	ГЗ_ИзвещениеОПроведенииЗакупок.Ссылка = &Ссылка
		|
		|СГРУППИРОВАТЬ ПО
		|	ГЗ_ИзвещениеОПроведенииЗакупок.purchaseObject";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		ИначеЕсли  МассивОКПД2.Количество() = 3 Тогда 
			//Движение.ОКПД2_3Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]+"."+МассивОКПД2[1]+"."+МассивОКПД2[2]);
			Движение.ОКПД2_2Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]+"."+МассивОКПД2[1]);
			Движение.ОКПД2_1Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]);
	    ИначеЕсли  МассивОКПД2.Количество() = 2 Тогда 
			//Движение.ОКПД2_3Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]+"."+МассивОКПД2[1]+"."+МассивОКПД2[2]);
			//Движение.ОКПД2_2Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]+"."+МассивОКПД2[1]);
			Движение.ОКПД2_1Уровень = Справочники.КлассификаторОКПД2.НайтиПоКоду(МассивОКПД2[0]);
		...
		Движение.ДатаОкончанияПодачиЗаявок = Ссылка.collectiing_endDate;
		...
		ДвижениеПоРегионам.ДатаОкончанияПодачиЗаявок = Ссылка.collectiing_endDate;
