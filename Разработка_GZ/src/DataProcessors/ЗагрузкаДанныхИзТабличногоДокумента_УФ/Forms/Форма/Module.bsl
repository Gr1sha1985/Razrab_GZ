...
&НаКлиенте
Процедура КГИ_ПроверитьНоменлатуруНаУдаление(Команда)
	КГИ_ПроверитьНоменлатуруНаУдалениеНаСервере();
КонецПроцедуры

&НаСервере
Процедура КГИ_ПроверитьНоменлатуруНаУдалениеНаСервере()
	Для Каждого СтрокаТЧ Из ТабличныйДокумент Цикл
